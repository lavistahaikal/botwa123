...
try {
		...
		cmhit.push(command)
        mess = {
			wait: 'Bentar ya, DUMBOT sedang memproses.',
			success: 'Berhasil',
			error: {
				stick: 'Mohon maaf kak, DUMBOT tidak dapat mengakses video kamu.',
				Iv: 'Link yang kamu masukin salah!',
                api: 'Aghh, maaf kak ada kesalahan atau error.'
			},
			only: {
				group: 'Fitur ini hanya dapat dilakukan di group!',
				ownerG: 'Hoho maaf kamu bukan owner group!',
				ownerB: 'Hoho maaf kamu bukan owner akuuu!',
				admin: 'Kamu bukan admin group!',
				Badmin: 'Kamu tolol ya? Jadiin DUMBOT jadi admin duluu!'
			}
		}
		const botNumber = denz.user.jid
		const ownerNumberr = ["6283809250287@s.whatsapp.net",`${NomorOwner}@s.whatsapp.net`]
		...
		const katalog = (teks) => {
             res = denz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© FsyahNotFysah_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             denz.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'bit.ly/ownerpbotpoxcrafts', "groupName": `${NamaBot}`, "footerText": "*_© FsyahNotFysah_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
denz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© FsyahNotFysah_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: thubb, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",previewType:"PHOTO",thumbnail:thubb,sourceUrl:`https://youtube.com/channel/UCv8uiDsExEyVqdurq30gXmg`}}})
		}
		const simir = (teks) => {
...
reply(e)
        ...
        if (isMuted){
            ...
            if (budy.toLowerCase().startsWith(`${prefix}unmute`)){
                ...
                reply('DUMBOT telah diunmute di group ini')
            }
        }
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
				...
				if (isGroupAdmins) return reply('Waw ada bang admin <3')
				denz.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Yaaa, ngemis member :(')
			    denz.groupRemove(from, [kic]).catch((e) => { reply(mess.only.Badmin) })
			}
			}
			if (bad.includes(messagesD)) {
				reply('_Jangan toxic dong, banyak anak kecil!_')
				}
...
denz.updatePresence(from, Presence.recording)
   ...
   const sotoy = [
     ...
     case "5b017c6ac1fb953c7bd21034d2fca5fad75ef2da4c3b2c2877ef49fa544e74bf":
        ...
        case 'menu':
        case 'help':
        ptod = "6285387059588@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
...
│ ⬡ Nomor Owner : wa.me/6283809250287
└❏

╭─❒ 「 User Info 」 ❒
│ ⬡ Role : ${isOwner ? 'Owner' : 'Pengguna'}
│ ⬡ Nama : ${pushname}
│ ⬡ Bio : ${stst}
│ ⬡ Nomor : @${stod.split('@')[0]}
└❏
...
`
sendButLocation(from, `${menu}`, "*_© FsyahNotFysah_*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`${prefix}script`,buttonText:{displayText:'SCRIPT BOT'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
...
case 'command':
 ...
 footerText: '*_© FsyahNotFysah_*',
 description: `Hallo, silahkan pilih fiturnya :)`,
 sections: [
                     {
...
menu = `❏ 「 \`\`\`THANKS TO\`\`\` 」
├────────────────────
├ Developers Bot Indonesia
├ Penyedia Case Gratis
├ Allah S.W.T
├ Owner : ${NamaOwner}
├ All Creators Bot
└ All Members`
...
//Hardianto nsfw
case 'yuri':
if (!isNsfw) return reply(`Fitur NSFW sepertinya belum diaktifkan!\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
reply(mess.wait)
...
break
		...
		case 'sc':
		case 'sourcecode':
		denz.sendMessage(from, { text: "https://youtu.be/22NydDxIfDc", matchedText: 'https://youtu.be/22NydDxIfDc', description: "", title: "Click here🔥!!", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://youtu.be/22NydDxIfDc"
        },
        "index": 0
      }
    ]
...
break
    ...
    if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedVideo ) && args.length == 0) {
            ...
            reply('Hmm, reply gambar/video duluuu!')
            }
            break
case 'viewonce':
res = await denz.prepareMessageFromContent(from,{
...
break
case 'pinterest':
if (!c) return reply('Mau cari apa sih? Tulis yang mau dicariii!')
reply(mess.wait)
...
if (error){ return reply('_[ ! ] Upss, maaf terjadi kesalahan. Mungkin error atau muka kamu terlalu ganteng._')}
else {
var gugIm = result
...
return reply('_[ ! ] Upss, error/pencarian tidak berhasil!_') 
}) 
...
_Eh karena size file kamu terlalu besar, kamu mandiri aja yaaa tekan link diatas._`)
reply(mess.wait)
teks = args.join(' ')
...
_Sabar yaa, file kamu sedang dikirim. Estimasi ga sampai 5 menit._`
reply(result)
...
case 'tts':
		    ...
		    denz.groupDemoteAdmin(from, [demote])
						reply('Yaa kasihan pangkat kamu turun :v')
						break
					case 'promote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
		    ...
		    denz.groupMakeAdmin(from, [promote])
						reply('Selamat untuk yang dipromote menjadii admin <3')
						break
				case 'linkgrup':
				case 'linkgroup':
				case 'linkgc':
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				...
				break
				case 'tomp4':
					if (!isQuotedSticker) return reply('Reply stiker gif nya')
                                        reply(mess.wait)
            ...
            case 'tomp3':
					denz.updatePresence(from, Presence.composing)
					if (!isQuotedVideo) return reply('Videonya di-reply, jangan cuma dilihatin!')
					reply(mess.wait)
...
▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒`, MessageType.extendedText,{
 quoted: {
  ...
  message: {
    ...
    surface: 1,
    message: '☠️Fsyahh☠️',
    orderTitle: 'FsyahhVirus', // Idk what this does
